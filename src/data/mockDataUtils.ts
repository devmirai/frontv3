...
• Familiaridad con testing de APIs`,
  ...
  return Array.from({ length: count }, (_, i) => {
    ...
    closeDate.setDate(
      closeDate.getDate() + Math.floor(Math.random() * 60) + 15,
    );
    ...
    return {
      ...
      descripcion:
        jobData.description ||
        `Descripción detallada para ${jobTitles[i % jobTitles.length]}`,
  ...
  if (userRole === Rol.USUARIO) {
    return {
      ...
      stats: {
        ...
        completedInterviews: getApplicationsByUser(userId).filter(
          (app) => app.estado === EstadoPostulacion.COMPLETADA,
        ).length,
        pendingApplications: getApplicationsByUser(userId).filter(
          (app) => app.estado === EstadoPostulacion.PENDIENTE,
        ).length,
    ...
    return {
      ...
      stats: {
        ...
        pendingReviews: allApplications.filter(
          (app) => app.estado === EstadoPostulacion.PENDIENTE,
        ).length,
        completedInterviews: allApplications.filter(
          (app) => app.estado === EstadoPostulacion.COMPLETADA,
        ).length,
      },
    };
...
export const getMockPostulacionesByConvocatoria = (convocatoriaId: number) => {
  return mockApplications.filter(
    (app) => app.convocatoria?.id === convocatoriaId,
  );
};
...
export const generateMockQuestions = (
  ...
  const baseQuestions = [
    ...
    },
    {
      id: 3,
      texto:
        "Describe un proyecto técnico desafiante que hayas completado recientemente",
      tipo: "Technical",
      dificultad: 5,
    },
    {
      id: 4,
      texto:
        "¿Cómo te mantienes actualizado con las nuevas tecnologías en tu campo?",
      tipo: "Technical",
      dificultad: 3,
    },
    {
      id: 5,
      texto:
        "¿Cuáles consideras que son tus fortalezas principales para este rol?",
      tipo: "Behavioral",
      dificultad: 2,
    },
    {
      id: 6,
      texto:
        "Explica un momento en el que tuviste que aprender una nueva tecnología rápidamente",
      tipo: "Behavioral",
      dificultad: 4,
    },
    {
...
export const simulateInterviewEvaluation = (
  ...
  const scores = answers.map((answer, index) => {
    ...
    const hasKeywords =
      answer.toLowerCase().includes("experiencia") ||
